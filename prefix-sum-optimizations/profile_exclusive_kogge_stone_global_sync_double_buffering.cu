...
void kogge_stone_exclusive_scan_no_optimizations(uint32_t* input_array, uint32_t* output_array, int num_of_elements) {
    int block_size = 128;
    int num_blocks = (num_of_elements + block_size - 1) / block_size;
    uint32_t* block_sums;
    uint32_t* block_sums_temp;
    cudaMalloc(&block_sums, num_blocks * sizeof(uint32_t));
    cudaMalloc(&block_sums_temp, num_blocks * sizeof(uint32_t));
    kogge_stone_exclusive_scan_no_optimizations_kernel<<<num_blocks, block_size>>>(input_array, output_array, block_sums, num_of_elements);
    if (num_blocks > 1) {
        kogge_stone_exclusive_scan_no_optimizations(block_sums, block_sums_temp, num_blocks);
        add_block_sums_to_exclusive_output_kernel<<<num_blocks, block_size>>>(output_array, block_sums_temp, num_of_elements);
    }
    cudaDeviceSynchronize();
    cudaFree(block_sums);
    cudaFree(block_sums_temp);
}

void kogge_stone_exclusive_scan_global_sync_double_buffering(uint32_t* input_array, uint32_t* output_array, int num_of_elements) {
    int block_size = 1024;
    int shared_memory_size = 2 * block_size * sizeof(uint32_t);
    int num_blocks = (num_of_elements + block_size - 1) / block_size;
    ...
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_blocks_per_sm, kogge_stone_exclusive_scan_global_sync_double_buffering_kernel, block_size, shared_memory_size);
    ...
    cudaError_t result = cudaLaunchCooperativeKernel((void*)kogge_stone_exclusive_scan_global_sync_double_buffering_kernel, grid_size, block_size_dim, args, shared_memory_size);
...
void kogge_stone_exclusive_scan_cpu(uint32_t* input_array, uint32_t* output_array, int num_of_elements) {
    output_array[0] = 0;
...
int main()
{
    ...
    for (int n = lower_bound; n <= upper_bound; n += step_size)
    {
        ...
        kogge_stone_exclusive_scan_cpu(host_input_array, reference_array, n);
        ...
        kogge_stone_exclusive_scan_global_sync_double_buffering(device_input_array, device_output_array, n);
