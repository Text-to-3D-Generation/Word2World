...
void kogge_stone_inclusive_scan_global_sync_double_buffering(uint32_t* input_array, uint32_t* output_array, int num_of_elements) {
    int block_size = 1024;
    ...
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_blocks_per_sm, kogge_stone_inclusive_scan_global_sync_double_buffering_kernel, block_size, shared_memory_size);
    ...
    cudaError_t result = cudaLaunchCooperativeKernel((void*)kogge_stone_inclusive_scan_global_sync_double_buffering_kernel, grid_size, block_size_dim, args, shared_memory_size);
...
int main()
{
    ...
    for (int n = lower_bound; n <= upper_bound; n += step_size)
    {
        ...
        kogge_stone_inclusive_scan_global_sync_double_buffering(device_input_array, device_output_array, n);
