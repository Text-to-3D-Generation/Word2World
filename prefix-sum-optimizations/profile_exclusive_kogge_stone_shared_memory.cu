...
void kogge_stone_exclusive_scan_shared_memory(uint32_t* input_array, uint32_t* output_array, int num_of_elements) {
    int block_size = 256;
    int shared_memory_size = block_size * sizeof(uint32_t);
    int num_blocks = (num_of_elements + block_size - 1) / block_size;
    uint32_t* block_sums;
    cudaMalloc(&block_sums, num_blocks * sizeof(uint32_t));
    kogge_stone_exclusive_scan_shared_memory_kernel<<<num_blocks, block_size, shared_memory_size>>>(input_array, output_array, block_sums, num_of_elements);
    if (num_blocks > 1) {
        kogge_stone_exclusive_scan_shared_memory(block_sums, block_sums, num_blocks);
        add_block_sums_to_exclusive_output_kernel<<<num_blocks, block_size>>>(output_array, block_sums, num_of_elements);
    }
    cudaDeviceSynchronize();
    cudaFree(block_sums);
}
...
int main()
{
    ...
    for (int n = lower_bound; n <= upper_bound; n += step_size)
    {
        ...
        kogge_stone_exclusive_scan_shared_memory(device_input_array,
