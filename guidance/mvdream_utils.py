...
class MVDream(nn.Module):
    def __init__(
        ...
        # Set half precision as our default type.
        self.dtype = torch.float16

        # Build the model and convert it to half precision.
        ...
        # Pass the half precision dtype to the scheduler so that it works with FP16 tensors.
    ...
    def encode_text(self, prompt):
        # prompt: [str]
        # Ensure the obtained embeddings are in FP16.
        embeddings = self.model.get_learned_conditioning(prompt).to(self.device).to(self.dtype)
        return embeddings
    ...
    def refine(self, pred_rgb, camera,
        ...
        # Convert pred_rgb_256 to half precision before encoding.
        latents = self.encode_imgs(pred_rgb_256.to(self.dtype))
        # latents = torch.randn((1, 4, 64, 64), device=self.device, dtype=self.dtype)

        self.scheduler.set_timesteps(steps)
        init_step = int(steps * strength)
        # Ensure that noise is generated in FP16.
        latents = self.scheduler.add_noise(latents, torch.randn_like(latents).to(self.dtype), self.scheduler.timesteps[init_step])

        # Convert camera to blender convention and normalize.
    ...
    def train_step(
        ...
        if as_latent:
            ...
            # Interpolate to 256x256 to be fed into the VAE.
            pred_rgb_256 = F.interpolate(pred_rgb, (256, 256), mode="bilinear", align_corners=False)
        ...
        # Process camera tensor to match blender convention.
        ...
        # Predict noise using the model (no grad computation).
        with torch.no_grad():
            noise = torch.randn_like(latents).to(self.dtype)
            latents_noisy = self.model.q_sample(latents, t, noise)
            latent_model_input = torch.cat([latents_noisy] * 2)
            
            # Convert t to float (and then to self.dtype) after concatenation:
    ...
    def prompt_to_img(
        ...
        # Generate text embeddings and prepare image latents.
